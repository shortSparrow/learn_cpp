...
int main() {
    /**
     * Проблема індексів у масивах під час ітерацій пов'язана з тим, щоб .size() повертаж unsigned число типк std::size_t.
     *
     ...
     * типом std::size_t який може бути більши за int тож я int може не покрити весь масив, тому правильніше було б
     * робити перевторення на std::ptrdiff_t
     *
    ...
    /**
     * .size() - повертає беззнакове чило типу std::size_t
     *
     ...
     * Але краще взагалі не використовувати індекси вручну, карще скористатися range або iterators
     */
