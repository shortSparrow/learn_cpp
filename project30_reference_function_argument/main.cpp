 ...
 *
 * Додатково є правило, що якщо об'єкт займає більше двох комірок пам'яті, то його буде швидше передати через reference, є
...
int main() {
    ...
    /**
     * Коли ми передаємо інший тип то спочатку створить тимчасовий об'єкт відповідного типу,
     * а потім він передасться (скопіюється якщо це просто передача аргументу чи передасться
