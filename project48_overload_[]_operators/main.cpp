...
int main() {
    ...
    /**
     *
     * З pointer так не спрацює, оскільки коли мова про pointer то для оператора []
     ...
     *
     * Але оскільки компілятор не може перетворити list у число (ми не написали конструктор, який прийма би int як аргумент - IntList(int x))
     * Я хуй зна нашо йому перетворювати list(IntList) на число, там є якась логіка, але я її не розумію, алк якщо додати конструктом
     * IntList(int x) то помилка зникне, але все одно результат буде непередбачуваний
     *
     *
     * Загалом НЕ можна перевантажувати оператор [] на pointer різних об'єктів
     */
