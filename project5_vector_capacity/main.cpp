...
int main() {
     ...
     * 8*8*8 = 24
     * А от щоб дізнатися скільки пам'яті за'має вектор треба sizeof(arrayElementType) * array.capacity()
     *
     * std::vector<bool> - це виключення, він має додаткові поля тому його контейнер важить більше (він не є
     * справжнім контейнером у C++). std::vector<bool> взагалі краще уникати, бо такий код з ним працювати не буде:
     *
     * template<typename T>
     ...
     * Коли ми робимо resizing vector для збільшення то збільшується його length і capacity, однак
     * якщо ми змінюємо вектор через resizing то зменшиться лише його length, а capacity лишиться старим.
     * Однак, якщо у нас був дуже великий масив і ми все таки хочемо вивільнити пам'ять, то треба використати shrink_to_fit
     *
     */
    std::cout << "................Resizing..................\n";
