 ...
 *
 *  Коли ми пишемо cout << "" або return "some string" (якщо не зазначено тип, або ти char), то це C-style string
 */
...
int main() {
    ...
    // Так само як і C-style array можна визначити, але не можна записати нове значення змінній
    char str3[]{ "string" };
    // str3 = "rope";        // error
     ...
     * для цього випадку можна використати функцію std::strlen(), вона працює для pointer і C-style string до decay
     *
     */
    std::cout << "********************** get size ******************" << '\n';
     ...
     * масивом char і закінчується '\0' (null terminator), тож виводить все від pointer на який вказує
     * масив (перший елемент масиву), аж до поки не зустріне null terminator. От тільки в варіанти з c
     * null terminator немає, бо нагадую ще ращ, що це не масив, а один char. Тож буде виводитися всяке сміття,
     * яке лежить в пам'яті після pointer аж до поки не зустріне null pointer. В маємо випадку "сміття" це
     * рядок "string" який лежить після змінної c.
     *
     * Якщо ж ми все таки хочемо вивести адресу c то можна скористатися  static_cast<const void*>(ptr)
