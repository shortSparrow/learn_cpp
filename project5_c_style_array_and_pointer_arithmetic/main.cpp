...
int main() {
     ...
     *
     *
     * Компілятор перетворює ptr[n] в *((ptr) + (n))
     ...
     *  перший елемент масиву *(arr + 0) = 0xf3163ff7e4 + 0 = 0xf3163ff7e4
    ...
    /**
     * Наочна ілюстрація, що зазвичай pointer показує на перший елемент масиву
    ...
    /**
     * Цікавий факт, що для C-style array від'ємний індекс це цілком собі нормальна річ, бо
     * arr[-1] перетвориться у *(ptr -1) і поверне попередній об'єкт у пам'яті
     */
    std::cout << "************************** arr[-1] *******************" << '\n';
