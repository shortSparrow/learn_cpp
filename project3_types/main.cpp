 ...
 * Стандарт С++ не визначає точного значення, скільки пам'яті має алокувати тип даних, натомість стандарт говорить лише про
...
int main() {
    ...
    //
    //
    // // Unsigned не радять використовувати, одна з причин - дуже легко їх переповнити
    // unsigned int x{ 2 };
    // unsigned int y{ 3 };
    //
    ...
    /**
     * float має 7 значущих цифр які зберігає, а double - 9
     ...
     * 1.2345678901 закінчується на 1 і замість 1 ми отримали 0999999, а числа до того збережені правильно
     * Тож треба бути обережним з останнім знаком, бо навіть якщо тип даних може його вмістити його може
     ...
     *
     * Але тут помилка саме у setprecision, бо якщо вказати setprecision(9) то я отримаю 1.23456789 для double
     */
