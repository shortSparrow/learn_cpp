 ...
 * і оскільки double через numeric conversion перетвориться на int я втрачу частину інформації
 ...
 *
 * Важливо розуміти що template створює перевантаження функції на етапі компіляції, тобто
 * якщо я отримую з сервера дані у форматі long і закину їх у max то вони приведуться до int,
 * бодо компіляції template не знає що треба робити якесь перевантаження окрім double.
 *
 *
 *
 */
...
int main() {
     ...
     * Припустимо буде функція з template і перевантажена для bool яку я додаю сам і вона буде по своєму виводити
     ...
     *   std::cout << '\n';
     *
     * Як бачимо все що має <> не бачить створену мою функцію яка по особливому виводить bool, натомість вони строюють свій варіант
