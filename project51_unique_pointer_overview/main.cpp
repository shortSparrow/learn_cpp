...
int main() {
    ...
    /**
     * Але цей клас має один великий недолік, ми не перевизначили оператор = і констурктор копіювання,
     * а отже компілятор створить дефолті за нас, і тут виникне проблема. У прикладі нижче викличиться
     * констурктор копіювання.
     *
     ...
     * Повернути як pointer (Resource*), (ми сторимо об'єкт десь у heap (купі) і повернемо цого адресу) але тоді ми можемо забути його очистити пізніше
     * Повернути як значення, але це зробить дорогу копію і втрачається сенс smart pointer
     *
     * Правильним варіантом буде Move semantics - ми премістимо володіння об'єктом до ішного власника, а у поточного видалимо
     *
     */
