...
int main() {
  /**
   * У мене є масив string_view і lambda function і коли я шукаю статично підрядок то все ок.
  ...
  */
  ...
  /**
   * Mutable captures
   * Коли ми викликаємо shoot2 то вона створює копію ammo2 і ця копія живе своє життя, тому що captured variables
   * є членами lambda об'єкту, а їхні значення зберігаються і доступні для інших викликів lambda! Це чимось схоже на
  ...
  // Capture by reference. Якщо будемо передавати змінну як reference то тоді можемо її змінювати
  int ammo3{ 10 };
   ...
   * Змінні захоплюються (captured) у лямбда, коли лямбда створена.
   ...
   *
   * Спосіб виправити - це дати тип лямбді такий самий, як і у параметра myInvoke - std::function
   * Або другий спосіб - це обгорнути count при передачі як аргумент у reference wrapper, який робиться
   ...
   * як аргумент, то вони будуть копіюватися. Саме тому варто використовувати std::ref, що передавати їх
