...
int main() {
     ...
     * дії і повернулося значення, однак після того, як функція add відпрацює, то всі її
    ...
    // це функція яка приймає два параметри і повертає int
...
// void як параметр означає що функція не приймає параметр, але це застарілий підхід, краще
// писати int some_func()
void some_func(void) {
    int a= 12;
}