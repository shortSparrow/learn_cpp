...
int main() {
    ...
    /**
     * Коли у нас є overload функцій і типи збігаються як у прикладі вище все просто,
     * функцію з параметрами double викликаємо add(1.1,2.0), а з int  add(1,2),
     * але що робити коли типи не збігаються.
     *
     * Компілятор запускає type conversion до аргументів кожної функції і дивиться чи щось підійшло.
     * Можливі чотири результати:
     *      1. Type conversion не дав збігів, тому рухаємося до наступної функції. Як відбувається перевірка:
     ...
     *      2. Збіг по Type conversion знайдено, подальші функції не перевіряються
     *      3. Кілька збігів знайдено. Компілятор видасть помилку, бо не знає яку функцію треба обрати
     *      4. Жодна функція не дала збігу по Type conversion. Компілятор видає помилку
     */
