 ...
 * одночасність читання структури даних, коли вона не піддається модифікації; необхідний інший тип м'ютекса.
 * Цей новий тип м'ютекса зазвичай називають м'ютексом читання-запису, оскільки він дозволяє два різних типи
 * використання: ексклюзивний доступ одного потоку «запису» або спільний і одночасний доступ декількох потоків «читання»
 *
 * Стандартна бібліотека C++17 надає два таких м'ютекси, std::shared_mutex і std::shared_timed_mutex.
 *
 *
