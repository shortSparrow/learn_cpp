 ...
 * Костиль із wait_for(1ms) змушував процесор постійно прокидатися (polling).
 ...
 * викликає flag->set(), який в свою чергу блокується м'ютексом set_clear_mutex у флазі interruption_flag, а ми
 ...
 * Якщо ми реєструємо cv у прапорці, ми повинні гарантувати, що коли cv знищується (наприклад,
 * функція завершилася), прапорець більше не тримає на неї вказівник. Інакше interrupt()
 * стрільне в сміттєву пам'ять (Undefined Behavior). Для цього використовують структуру-охоронець
 *
 * Результат:
 ...
 * що у флага interruption_flag, якщо навпаки зовнішній розблоковується, то і внутрішній так само.
 */
